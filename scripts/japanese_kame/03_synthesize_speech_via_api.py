...
Usage:
    uv run --extra tts -m scripts.japanese_kame.03_synthesize_speech_via_api \
        --service_account llmjp-497505-118510c984bd.json \
...
def _build_client(service_account_path: str | None, location: str):
    import os
    ...
    if api_key:
        # Google AI Studio mode — no Vertex AI API required
        return genai.Client(api_key=api_key)

    if not service_account_path:
        raise ValueError("Either GEMINI_API_KEY env var or --service_account must be provided.")
    ...
    # Requires Vertex AI (aiplatform.googleapis.com) to be enabled in the project
    return genai.Client(
        vertexai=True,
        project=project_id,
        location=location,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="data/japanese_kame/audio_gemini")
    parser.add_argument("--model", type=str, default="gemini-3.1-flash-tts-preview")
    parser.add_argument(
        "--service_account",
        type=str,
        default=None,
        help="Path to Google Cloud service account JSON (Vertex AI mode). "
             "Alternatively set GEMINI_API_KEY env var to use Google AI Studio.",
    )
